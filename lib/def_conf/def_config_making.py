...
def config_making_main(ext_info, out_path = "", make_dir_index = False, info_list = [1,0,0,0], fp = ""):
    ...
    if(out_path != ""): # When the path for the config file output is entered
        ...
        # Create a directory for the output of the config file
    ...
    else: # When the path for config file output is not entered
        filename = "config.json"
        out_path = common_path + "/config"
        out_path, filename = check_filename(out_path, filename, info_list = info_list, fp = fp)
        
    # Display the output destination and file name of the config file
    ...
    # Define the parameters of the config file
