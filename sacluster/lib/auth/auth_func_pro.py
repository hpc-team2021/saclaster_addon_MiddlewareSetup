...
#def json schema in authentication
jsa_auth = {
       "type": "object",
       "required": [
        "access token",
...
def authentication_cli(fp = "", info_list = [1,0,0,1], api_index = True):
    ...
    if(os.path.exists(home_path + "/setting/" + filename) == True):   #<--windowsでは変更あり
        ...
        if(res == True):
            ...
            if(api_index == True):
                ...
                if("Account" not in res):
                    _ = printout("AuthenticationError: cannot access", info_list = info_list, fp = fp)
                    logger.error('AuthenticationError: cannot access')
                    auth_res = define_auth_info(home_path + "/setting/Setting.json", info_list = info_list, api_index = api_index, fp = fp)
